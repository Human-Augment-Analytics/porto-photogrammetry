...
# LOCAL PATCH (augenblick): setuptools >= 81 removes pkg_resources, and this repo's envs
# ship setuptools 83, so the upstream `import pkg_resources` raises ModuleNotFoundError at
# model-build time. Reinstalling an older setuptools would violate the rule that nothing
# already in the env gets changed, so shim the one API used here (resource_filename, called
# only to locate sam3/assets/bpe_simple_vocab_16e6.txt.gz) onto stdlib importlib.resources.
try:
    ...
    class _PkgResourcesShim:
        ...
        def resource_filename(package, resource):
            return str(_resources.files(package).joinpath(resource))
...
def _create_sam3_model(
    ...
    if not eval_mode:
        raise NotImplementedError(
            "eval_mode=False needs sam3.train.matcher, which is not vendored in this repo."
        )
...
def build_sam3_image_model(
    ...
    if enable_inst_interactivity:
        # LOCAL PATCH (augenblick): the tracker/video/multiplex modules are not vendored --
        # this tree is trimmed to the image-masking path. Fail loudly rather than NameError.
        raise NotImplementedError(
            "enable_inst_interactivity requires the SAM 3 tracker modules, which are not "
            "vendored in this repo (see .claude/PLANS/sam3-masking-method.md §1)")
    inst_predictor = None
