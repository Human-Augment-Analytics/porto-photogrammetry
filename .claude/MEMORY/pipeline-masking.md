...
- **`sam3`**: concept-prompted instance segmentation (SAM 3, Meta), GPU-only. A **text prompt**
...
- **bfloat16 autocast is mandatory and undocumented.** Without
...
- The checkpoint comes from the **gated** HF repo `facebook/sam3` (needs `hf auth login`).
  Downloading on a PACE login node gets OOM-killed by the cgroup cap; fetch it in a batch job.
  `.incomplete` blobs make `du` report progress while `snapshots/` is still empty, so size is
  not a completion check.

`src/libs/sam3` is a **trimmed vendored copy**: upstream tracking, video, training and eval
paths removed (284 files/11 MB → 31 `.py`/1.9 MB), leaving `sam3/{model,perflib,sam,assets}`.
Local deviations are marked `LOCAL PATCH (augenblick)` — chiefly a `pkg_resources` shim onto
`importlib.resources` (setuptools ≥ 81 dropped it) and two training-only branches raising
`NotImplementedError`. `perflib/fa3.py` looks unused statically but is lazily imported on
Hopper and must stay. Full prune list: `.claude/PLANS/sam3-masking-method.md`.
...
`rembg` infers at 1024² but returns a mask at **source resolution**, which `run()` passes
straight to `postprocess_mask` — so `keep_largest`/`fill_holes` do morphology on 26 MP, not
1 MP. On 6240×4160 images the per-image budget is ~2.2 s (L40S, over 4370 images with
`keep_largest` on; the default path now skips its 0.23 s):
...
The GPU is mostly idle, so a masking job is CPU-bound: more `--cpus-per-task` helps, a bigger
card does not. Postprocessing at model resolution and upscaling after would cut ~3-4x, but
shifts mask boundaries (low-res hole-filling can miss thin structures) — a behaviour change to
validate visually, not a free win. Verified against base.py and a timed 10-scene neurips run.

`sam3` is the exception at **1.60 s/image** on an L40S (432 images, 6240×4160) — faster
end-to-end than `rembg` despite the far larger model, because it does no `binary_fill_holes`.
