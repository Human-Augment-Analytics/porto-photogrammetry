# AliceVision counterpart of common.sh: Meshroom env, then scene_common.sh. It cannot use
# common.sh (one GPU-independent env, bundled libcudart) - see README.md.
...
# Both live under /blue here, not $HOME/scratch as on PACE.
export ALICEVISION_ROOT="${ALICEVISION_ROOT:-/blue/arthur.porto/srizvi63.gatech/alicevision}"
export MESHROOM_ROOT="${MESHROOM_ROOT:-/blue/arthur.porto/srizvi63.gatech/Meshroom}"
...
gpu_supported() {
    case "$1" in
        ...
        l40s)                   echo 89 ;;
        h100|h200)              echo 90 ;;
        rtx6000|rtx_pro_6000|rtx_pro_6000_blackwell) echo 120 ;;
        b200)                   echo 100 ;;
        mi210)                  echo -1 ;;
        *)                      echo 0 ;;
    esac
...
# --- conda ------------------------------------------------------------------
# AliceVision bundles its own libcudart; loading a CUDA module would shadow it.
module purge
